...
!      template_file: /space/mk5_merge3/include/solve.templ
!      local customization file: /space/mk5_merge3/local/solve.lcl
!
...
!  kdb    2009.10.06  Raised MAX_SUP from 20000 to 32000 again after an
!                     earlier change to do this was somehow lost.
!  kdb    2009.10.30  Increase MAX_SDC from 100 to 300. (Then also must
!                     increase JGLBC4_BLOCKS from 163 to 206 in this include
!                     file and must change LEN_GLBC4_FILLER in glbc4.i
!                     accordingly.)
!  kdb    2009.12.09  Parameterized cgm catalog file (CGMCAT_FILE).
!  dsm    2010.10.28  Increased JSOCOM_BLOCKS from 58 to 220
!  dsm    2010.10.28  Increased MAX_ATM from 300 to 1500
!  dsm    2010.10.28  Increased MAX_CLK from 100 to 1500
!  dsm    2010.10.28  Increased MAX_GRAD from 64 to 1500
!  dsm    2011.07.22  Increased MAX4_WEIREC from 98304 to 131072
!  jmg    2012.07.17  Added parameters assocaited with debug output and processing iono
!  jmg    2012.12.04  Added ivgosDB__NO, ivgosDB__YES
!  JMG    2013.04.18  Decreased number of gblc4 blocks
!  JMG    2013.04.24  Increased MAX4_WEIREC to 200000
!  JMG    2014.02.15  Added itimetag__UTC, itimetag__TAI
!  JMG    2015.02.24  Added REQ__TRP and USE__TRP, UNDF__TRP
!
!=====================
!
...
!
      ...
      PARAMETER ( MAX_PAR  = 32000 )
!
! --- Maximal number of arcs estimated in one solution.
!
      PARAMETER ( MAX_ARCS = 16384 ) ! Local customization
!
...
! --- GLBFIL is now 3281 blocks long = GLBCM(  17 blocks)
!                                    + GLBC2( 251 blocks)
!                                    + GLBC3(2807 blocks)
!                                    + GLBC4( 201 blocks)
!
...
!
     ...
     &            MAX_ATM=1500,     & ! max number of atmosphere segments
     &            MAX_CLK=1500,     & ! max number of clock segments
     &            MAX_GRAD=1500,    & ! max number of atmosphere gradient segments
...
!
      PARAMETER ( MAX_PLOTS = MAX_ARC_BSL*2                     )
...
!
      CHARACTER SPOOL_DIR*25
      PARAMETER ( SPOOL_DIR = '/space/solve/spool_files/' ) ! Local customization
      CHARACTER SCRATCH_DIR*5
      PARAMETER ( SCRATCH_DIR = '/tmp/' ) ! Local customization
      CHARACTER SUP_DIR_LIST*16
      PARAMETER ( SUP_DIR_LIST = 'super.dir.bootes' ) ! Local customization
      CHARACTER PHD_DIR*5
...
!
      CHARACTER CGM_DIR*23
      PARAMETER ( CGM_DIR = '/space/solve/cgm_files/' ) ! Local customization
      CHARACTER MK5_ROOT*5
      PARAMETER ( MK5_ROOT = '/mk5/' ) ! Local customization
      CHARACTER SOLVE_PROG_DIR*21
      PARAMETER ( SOLVE_PROG_DIR = '/space/mk5_merge/bin/' ) ! Local customization
      CHARACTER SOLVE_WORK_DIR*24
      PARAMETER ( SOLVE_WORK_DIR = '/space/solve/work_files/' ) ! Local customization
      CHARACTER SOLVE_SAVE_DIR*26
      PARAMETER ( SOLVE_SAVE_DIR = '/500/oper/solve_save_files' ) ! Local customization
...
!
      ...
      CHARACTER SUPCAT_FILE*16
      PARAMETER ( SUPCAT_FILE = 'super.cat.bootes' ) ! Local customization
      CHARACTER CGMCAT_FILE*13
      PARAMETER ( CGMCAT_FILE = 'CGMCAT.bootes' ) ! Local customization
!
...
!
!  Added by JMGipson 2013Nov15
! Some other lus
      integer*2, parameter :: lu_ubuf=34
      integer*2, parameter :: lu_debug=35
!
...
!
      INTEGER*4   ATD__UNDF, ATD__NO, ATD__INSTANT, ATD__AVERAGE, &
                  ATD__LAGGED, ATD__USE, ATD__REQ,ATD__EXTERNAL,ATD__EXTLAGGED
      ...
      PARAMETER ( ATD__EXTERNAL= 23006 )
      PARAMETER ( ATD__EXTLAGGED= 23007 )
 
      INTEGER*4   DISP__UNDF, DISP__FOUND
      PARAMETER ( DISP__UNDF    =     0 )
      PARAMETER ( DISP__FOUND   = 66000 )
!
      ...
      INTEGER*2, parameter :: ivgosDB__NO=0                    !Not using vgosDB format
      INTEGER*2, parameter :: ivgosDB__YES=1                   !Using vgosDB format.
      integer*2, parameter :: itimetag__UTC = 0                 !default
      integer*2, parameter :: itimetag__TAI = 1                 !use TAI
 
      integer*4,  parameter ::   UNDF__TRP=0
      integer*4,  parameter ::   REQ__TRP=1
      integer*4,  parameter ::   USE__TRP=2
 
      INTEGER*4, PARAMETER  :: iono__pre2012jul    = 1      !use default bug in correlation of ionosphere sigma with Obs sigma.
      integer*4, parameter  :: iono__nocorrelation = 2      !assume no correlation with observation
 
!
! The bit that is set...
      integer*2, parameter  :: idebug__loop__calib = 1      !print debug output from loops call to calib
      integer*2, parameter  :: idebug__loop__flyby = 2      !print debug outpuyt from loops call to flyby
      integer*2, parameter  :: idebug__loop__partl = 3      !print non-zero partials in loop.f
      integer*2, parameter  :: idebug__vgosDBread  = 4      !Do verbose mode for vgosDBread
      integer*2, parameter  :: idebug__loop__normal = 5      !Output normal matrix (non-fast mode) in loop at various steps
      integer*2, parameter   :: idebug__constrained__matrix= 6     !output matrix after constraints
...
!
      ...
      PARAMETER  ( SCRATCH_DATE = '2008.04.24' ) ! scratch files. Scratch files
